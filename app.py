...
if page == "Dashboard":
    ...
    suggested_prompts = [
        ...
        "Which items should we avoid promoting due to low stock?",
    ]
    for prompt_option in suggested_prompts:
        if st.button(prompt_option, key=f"suggest-{prompt_option}"):
            st.session_state["ai_prompt"] = prompt_option

    prompt = st.text_area("Ask a question", value=st.session_state.get("ai_prompt", ""))
    if st.button("Get Response") and prompt:
        with st.spinner("Thinking..."):
            response = generate_response(prompt, menu_items, inventory, orders)
        st.write(response)
